...
function renderNav(page) {
  ...
  const links = ["prateleira", "sistema", "comparar", "importar"];
...
function renderButton(idDestino, idButton) {
  const destino = document.getElementById(idDestino);
  const addButton = document.createElement("button");
  addButton.type = "button";
  addButton.textContent = `botão ${idButton}`;
  addButton.classList.add("btn", "btn-success", "btn-lg");

  // Corrigido: função anônima para só executar no clique
  addButton.onclick = () => escrever(idButton);

  destino.appendChild(addButton);
}
